    ...
    if not employment_duration or not cred_hist_length or customer_income <= 0 or loan_amnt <= 0:
        ...
        try:
            ...
            if pred == 1:
                ...
                st.subheader("💡 Detailed Analysis of Rejection")
                st.write("The following factors failed the AI safety threshold:")

                reasons = []
                if user_data_raw["historical_default"] > 0:
                    reasons.append(
                        "• **Historical Default**: History of default significantly increases lending risk profile.")
                if user_data_raw["customer_income"] < (user_data_raw["loan_amnt"] / 4):
                    reasons.append(
                        "• **Debt-to-Income**: Requested loan amount is disproportionately high compared to income.")
                if user_data_raw["loan_int_rate"] > 12.0:
                    reasons.append(
                        "• **Interest Rate**: The high rate leads to higher default probability in simulations.")
                if user_data_raw["employment_duration"] < 12:
                    reasons.append(
                        "• **Job Stability**: Employment duration under 1 year indicates potential income instability.")
                if user_data_raw["loan_grade"] >= 3:
                    reasons.append(
                        "• **Risk Grade**: The assigned credit grade falls below the minimum approval standard.")

                if not reasons:
                    reasons.append(
                        "• **Risk Profile**: Combined financial variables do not meet the minimum safety score.")
